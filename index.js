...
function buildPDF(rows) {
  return new Promise((resolve, reject) => {
    ...
    function drawTagList(label, items) {
      ...
      const CHIP_MAX_W = 110;  // max chip width before wrapping text inside
      ...
      // Pre-compute each chip's width and height based on actual text layout
      const chipMeta = items.map(item => {
        const singleLineW = doc.font("Helvetica-Bold").fontSize(7).widthOfString(item);
        const cw = Math.min(singleLineW + PAD_X * 2, CHIP_MAX_W);
      ...
      // Wrap chips into lines by available width
      const lines = [[]];
      let lineW = 0;
